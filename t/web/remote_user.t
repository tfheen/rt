...
diag "WebRemoteUserAutocreateDefaults";
{
    RT->Config->Set( DevelMode => 0 );
    RT->Config->Set( WebRemoteUserAuth => 1 );
    ...
    RT->Config->Set( WebRemoteUserAutocreateDefaults => { Organization => "BPS" } );
    ...
    diag "New user";
    {
        ...
        is $user->Organization, "BPS", "Found Organization from WebRemoteUserAutocreateDefaults hash";
        ok $user->Privileged, "Privileged by default";
    }

    stop_server(\$m);
    RT->Config->Set(
        WebRemoteUserAutocreateDefaults => {
            Privileged   => 0,
